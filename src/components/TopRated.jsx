...
const TopRated = () => {
    const [{rated,details},dispatch] = useStateValue();
    ...
    useEffect(()=>{
        const fecthData = async() =>{
          ...
          const rated = await Promise.all(
            ...
            console.log("toprated data:",rated);
        }
        fecthData();    
    },[]);
  ...
  return (
    ...
    <h1 className="text-2xl font-bold mb-8">Top Rated Movies</h1>
    <Swiper
      slidesPerView={3}
      spaceBetween={30}
      // scrollbar={{ hide: true }}
      className="mySwiper"
    >
      {rated?.map(({ id, title, original_title, backdrop_path, poster_path, overview, vote_average, video, release_date }) => (
